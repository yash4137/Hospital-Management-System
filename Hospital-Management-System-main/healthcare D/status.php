...
<body>
    ...
    <div class="pagename">
        <center>Appointment Status</center>
    </div>
    ...
    <table class="table">
        ...
        <?php
            include('connect.php');
            $sql = "SELECT * FROM `patient.information`";
            $result = $con->query($sql);

            if($result->num_rows > 0) {
                while($row = $result->fetch_assoc()) {
                    echo "<tr><td>" . $row["name"] . "</td><td>" . $row["number"] . "</td><td>" . $row["email"] . "</td><td>" . $row["doctor"] . "</td><td>" . $row["datetime"] . "</td><td>";
                }
            } else {
